...
#define REC_WINDOW 5000	// number of records to read at a time
...
int main(int argc, char* argv[]) {
    ...
    char   sandbox[REC_LENGTH * REC_WINDOW], dbname[64];
    ...
    total_size += sizeof(char)*REC_LENGTH*REC_WINDOW;
#pragma omp target data map(alloc:sandbox) map(to: target_lat, target_long)
#endif
    {
    while(!done) {
...
#ifdef OMP_OFFLOAD
//#pragma omp target data map(z, sandbox)
#ifdef OMP_OFFLOAD_NOREUSE
    total_size += sizeof(char)*REC_LENGTH*REC_WINDOW + sizeof(float)*REC_WINDOW
                    + sizeof(float)*2;
#pragma omp target data map(z, sandbox,target_long, target_lat)
#else
    total_size += sizeof(float)*REC_WINDOW;
//#pragma omp target data map(z)
#endif
#pragma omp target teams distribute parallel for
#endif
        //map(from: z) map(to: sandbox)
        //map(z, sandbox)
        //map(z[0:REC_WINDOW], sandbox[0: REC_LENGTH*REC_WINDOW])
        //map(always,from: z[0:REC_WINDOW]) map(always,to: sandbox[0: REC_LENGTH*REC_WINDOW])
        for (i = 0; i < rec_count; i++){
            char *rec_iter = sandbox+(i * REC_LENGTH + LATITUDE_POS - 1);
            float tmp_lat = atof(rec_iter);
            float tmp_long = atof(rec_iter+5);
            z[i] = (( (tmp_lat-target_lat) * (tmp_lat-target_lat) )+( (tmp_long-target_long) * (tmp_long-target_long) ));
        }
