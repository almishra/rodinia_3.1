...
#ifdef OMP_OFFLOAD
#pragma omp target data map(J[0:size_I]) map(to: dN[0:size_I], dS[0:size_I], dW[0:size_I], dE[0:size_I], c[0:size_I], rows, cols, iN[0:rows], iS[0:rows], jW[0:rows], jE[0:rows]) 
#endif
#ifdef ITERATION
...
#ifdef OPEN
#ifdef OMP_OFFLOAD
#pragma omp target teams distribute parallel for shared(J, dN, dS, dW, dE, c, rows, cols, iN, iS, jW, jE) private(i, j, k, Jc, G2, L, num, den, qsqr) 
#else
