...
void lud_perimeter(float *a, int size, int offset)
{
    int chunk_idx;
    ...
    #pragma omp parallel for default(none) \
      shared(size,chunks_in_inter_row, offset, a) 
...
void lud_internal(float *a, int size, int offset)
{
    int size_inter = size - offset -  BS;
    int chunks_in_inter_row  = size_inter/BS;
...
void lud_omp(float *a, int size)
{
    ...
    for (offset = 0; offset < size - BS ; offset += BS)
    {
        ...
        lud_perimeter(a, size, offset);
        lud_internal(a, size, offset);
    }
